    ...
    public partial class Order : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                HttpCookie httpCookie = Request.Cookies["PageCount"];
                if (httpCookie == null)
                {
                    httpCookie = new HttpCookie("PageCount");
                    httpCookie.Value = "1";
                    httpCookie.Expires = DateTime.Now.AddDays(5);
                }
                else
                {
                    int nPageCount = int.Parse(httpCookie.Value);
                    httpCookie.Value = (nPageCount + 1).ToString();
                    httpCookie.Expires = DateTime.Now.AddDays(365);
                }
                Response.Cookies.Add(httpCookie);
                lblPageHits.Text = httpCookie.Value;
